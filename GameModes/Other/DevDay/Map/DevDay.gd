...
func _ready():
	...
	var val : int = Config._loadSomethingFromConfig("GameModeDay", "SumCollected")
	if val != null:
		collectedSum = val
		_setSumPick(val)
...
func _spawnCollectable() -> void:
	var collectableScene = preload("res://GameModes/Other/DevDay/Map/Collectable.tscn")
...
func _setSumPick(var val : int) -> void:
	Config._saveSomethingInConfig("GameModeDay", "SumCollected", val)
